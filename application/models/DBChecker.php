...
class DBChecker extends CI_Model {

	function TabelHariIni($perusahaan)
	{
		$now = date("Y-m-d");
		$query 		= "SELECT anggaran.*, akun.*, anggaran.id AS idanggaran FROM anggaran, akun WHERE anggaran.akun = akun.id AND anggaran.perusahaan = $perusahaan AND anggaran.tanggal = '$now' AND anggaran.status NOT IN ('approved') ORDER BY anggaran.id DESC";
	...
	function TabelBulanIni($perusahaan)
	{
		...
		$start = $now . "-00";
		$end = $now . "-31";
		$query 		= "SELECT anggaran.*, akun.*, anggaran.id AS idanggaran FROM anggaran, akun WHERE anggaran.akun = akun.id AND anggaran.perusahaan = $perusahaan AND anggaran.tanggal > '$start' AND anggaran.tanggal < '$end' AND anggaran.status NOT IN ('approved') ORDER BY anggaran.id DESC";
		return $this->db->query($query);
	}
